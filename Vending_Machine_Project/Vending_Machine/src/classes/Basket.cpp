...
bool Basket::add_item_basket(Item *item_add){
    ...
    if(item_count ==0){
        ...
        //item = item_add;
        item_count++;
        total_amount+=item_add->price*item_add->quantity; 
        return true;
    }
    else{
        for(auto it: items){
            if((it->name == item_add->name) && (it->get_attribute()==item_add->get_attribute())){
                items.at(i)->quantity += item_add->quantity;
                total_amount+=item_add->price*item_add->quantity; 
                return false;
            }
            i++;
...
bool Basket::remove_item_by_number(){
    if(total_amount == 0.0){
        ...
        item_count--;
        total_amount-= items.at(*remove_number-1)->price * items.at(*remove_number-1)->quantity;
...
void Basket::print_items(){
    ...
    for(auto it: items){
        ...
        it->print_attribute(); 
        cout << "\t\t\t" << it->quantity 
             << "\t" << setw(7) << right << it->price*it->quantity << "€ " << endl;
        i++;
    }
    if(i==1){
...
void Basket::print_items_file(ofstream& my_file){
    for(auto it: items){
        ...
        my_file << "\t\t\t" << setw(4) << right << it->quantity 
                << "\t" << setw(7) << right << it->price*it->quantity << "€ " << endl;
    }
}
